...
from bottleserver import *

# Setup HTTP server
httpserver = BottleServer(host='', port=4000, rootdir='/home/tom/sensorserve/static', debug=True)
httpserver.loop()
...
server = EchoServer(host='', port=2005) # set host to localhost to limit access
...
while True:
	if connected == 1:
		try:
			...
			if networkmsg=='R':
			    server.senddata("Content-type: text/html\n\r"+content+'\n')
